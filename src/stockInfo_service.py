...
async def 현재상태(ctx):
    ...
    await ctx.send("🔄 Supabase에서 최신 상태 정보를 조회 중...")

    try:
        # 'status' 테이블의 가장 최근 데이터를 조회 (예시)
        # 사용자가 실제 테이블명과 컬럼에 맞게 수정해야 함
        response = supabase.table('status').select("*").limit(1).execute()
        ...
        # 데이터 필드 예시 (실제 컬럼명에 맞춰 수정 필요)
        latest_status = data[0]
        
        embed = discord.Embed(title="📊 현재 시스템 상태", color=0x3498db)
        
        # 딕셔너리의 모든 키-값 쌍을 필드로 추가
        ...
        await ctx.send(f"❌ 데이터 조회 중 오류 발생: {e}")
